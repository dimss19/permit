<x-app-layout>
    <x-slot name="title">Review Permit (Senior Manager)</x-slot>
    ...
    {{-- ===== FILTER BAR ===== --}}
    <form method="GET" action="/senior-manager/approvals" class="bg-white rounded-2xl border border-gray-100 shadow-sm px-6 py-4 mb-5 flex flex-wrap items-center gap-3">
        ...
        <select name="status" class="border border-gray-200 rounded-xl text-sm px-3 py-2 focus:outline-none focus:ring-2 focus:ring-inka-navy/20 focus:border-inka-navy text-gray-600">
            <option value="">Status: Review Sr. Manager</option>
            <option value="Active" {{ request('status') === 'Active' ? 'selected' : '' }}>Active (Sudah Disetujui)</option>
        ...
        <a href="/senior-manager/approvals" class="text-sm text-gray-400 hover:text-gray-600 transition-colors">
            Reset
        </a>
        @endif
    </form>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            <div>
                <h3 class="text-sm font-semibold text-gray-800">Daftar Antrean Final Review</h3>
                <p class="text-xs text-gray-400 mt-0.5">Permit yang telah disetujui Manager dan menunggu approval akhir Anda.</p>
            </div>
        </div>
        ...
        @if($permits->isEmpty())
            <div class="flex flex-col items-center justify-center py-16 px-6 text-center">
                ...
                <p class="text-sm font-semibold text-gray-700">Tidak ada permit yang perlu direview.</p>
            </div>
        @else
                ...
                <table class="w-full text-left text-sm">
                    <thead>
                        <tr class="text-xs text-gray-400 uppercase tracking-wide border-b border-gray-100 bg-gray-50/60">
                            ...
                            <th class="px-6 py-3 font-semibold">Tgl. Masuk Sr. Manager</th>
                            <th class="px-6 py-3 font-semibold text-right">Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-50">
                        @foreach($permits as $permit)
                        <tr class="hover:bg-orange-50/30 transition-colors cursor-pointer" onclick="window.location='/senior-manager/approvals/{{ $permit->id }}'">
                            <td class="px-6 py-3.5"><a href="/senior-manager/approvals/{{ $permit->id }}" class="font-semibold text-inka-navy hover:underline" onclick="event.stopPropagation()">{{ $permit->no_permit }}</a></td>
                            ...
                            <td class="px-6 py-3.5">
                                @if($permit->status === 'Review Senior Manager')
                                ...
                                @elseif($permit->status === 'Active')
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-green-100 text-green-700">Active</span>
                                @else
                            ...
                            <td class="px-6 py-3.5 text-gray-500 text-xs">{{ $permit->updated_at ? $permit->updated_at->format('d/m/Y') : '—' }}</td>
                            <td class="px-6 py-3.5 text-right">
                                @if($permit->status === 'Review Senior Manager')
                                <a href="/senior-manager/approvals/{{ $permit->id }}" class="inline-flex items-center bg-inka-navy text-white text-[11px] font-semibold px-3 py-1.5 rounded-lg hover:opacity-90 transition-colors">
                                    Review
                                </a>
                                @else
                                <a href="/senior-manager/approvals/{{ $permit->id }}" class="inline-flex items-center text-inka-navy text-[11px] font-semibold hover:underline">
                                    Lihat
                                </a>
                                @endif
                            </td>
