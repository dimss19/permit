<x-app-layout>
    <x-slot name="title">Tambah Akun Divisi</x-slot>
    ...
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Tambah Akun Divisi</h1>
                <p class="text-sm text-gray-500 mt-1">Buat akun baru untuk divisi atau pekerja</p>
            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <select name="name" required class="block w-full border border-gray-200 rounded-xl focus:ring-inka-navy focus:border-inka-navy sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors">
                                ...
                                @foreach($divisions as $division)
                                    <option value="{{ $division->name }}" {{ old('name') == $division->name ? 'selected' : '' }}>
                                        {{ $division->name }}
                                    </option>
                                @endforeach
                            </select>
                            <p class="text-xs text-gray-500 mt-1.5">Pilih divisi dari daftar yang tersedia. (Kelola di menu Kelola Divisi)</p>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="username" value="{{ old('username') }}" required 
                                ...
                                placeholder="Cth: teknik_div">
                            <p class="text-xs text-gray-500 mt-1.5">Username unik untuk login.</p>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="email" name="email" value="{{ old('email') }}" required 
                                class="block w-full border border-gray-200 rounded-xl focus:ring-inka-navy focus:border-inka-navy sm:text-sm px-4 py-3 bg-gray-50 focus:bg-white transition-colors" 
                                placeholder="Cth: teknik@inka.co.id">
                            <p class="text-xs text-gray-500 mt-1.5">Email kontak yang aktif.</p>
                        </div>
