<x-app-layout>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <a href="/admin/approvals"
           class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 hover:shadow-md hover:border-blue-200 transition-all group">
            ...
            <p class="text-3xl font-bold text-blue-500">{{ $counts['pending'] ?? 0 }}</p>
        ...
        <a href="/admin/approvals?status=Revision"
           class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 hover:shadow-md hover:border-red-200 transition-all group">
            <div class="flex items-center justify-between mb-3">
                <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide">Permit Direvisi</p>
            ...
            <p class="text-3xl font-bold text-red-500">{{ $counts['revision'] ?? 0 }}</p>
            <p class="text-[10px] text-gray-400 mt-1">Dikembalikan ke Divisi</p>
        </a>
        @endif
        ...
        <a href="/admin/history?status=Active"
           class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 hover:shadow-md hover:border-green-200 transition-all group">
            <div class="flex items-center justify-between mb-3">
                <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide">Active Hari Ini</p>
            ...
            <p class="text-3xl font-bold text-green-500">{{ $counts['active_today'] ?? 0 }}</p>
            <p class="text-[10px] text-gray-400 mt-1">Permit disetujui hari ini</p>
        </a>
        @endif
        ...
        <a href="/admin/approvals?date=today"
           class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 hover:shadow-md hover:border-gray-200 transition-all group">
            ...
            <p class="text-3xl font-bold text-gray-700">{{ $counts['today'] ?? 0 }}</p>
            <p class="text-[10px] text-gray-400 mt-1">Antrean masuk hari ini</p>
        </a>
        @endif
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm">
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            <div>
                <h3 class="text-sm font-semibold text-gray-800">Permit Menunggu Approval ({{ $roleName }})</h3>
                <p class="text-xs text-gray-400 mt-0.5">Urutan berdasarkan waktu pengajuan terlama</p>
            </div>
            <div class="flex items-center gap-3">
                <a href="/admin/approvals" class="text-sm font-semibold text-inka-navy hover:underline">
                    Lihat Semua
                </a>
                <a href="/admin/approvals"
        ...
        @if($permits->isEmpty())
            ...
            <div class="flex flex-col items-center justify-center py-16 px-6 text-center">
                ...
                <p class="text-sm font-semibold text-gray-700">Tidak ada permit yang menunggu review.</p>
                <p class="text-xs text-gray-400 mt-1 mb-6">Permit akan muncul di sini bila perlu ditindaklanjuti.</p>
            </div>
        @else
